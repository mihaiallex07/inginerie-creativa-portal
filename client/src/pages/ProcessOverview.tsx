...
// Fixed row height — bars stack inside
const BAR_H = 18;       // height of each project bar inside the cell
const BAR_GAP = 3;      // gap between bars
const ROW_PAD = 6;      // top+bottom padding inside cell
const MAX_BARS_VISIBLE = 4; // max bars before "+N more"
const ROW_H = ROW_PAD * 2 + MAX_BARS_VISIBLE * BAR_H + (MAX_BARS_VISIBLE - 1) * BAR_GAP; // ~90px
...
export default function ProcessOverview() {
  ...
  function syncScroll(source: "gantt" | "label", scrollTop: number) {
    if (source === "gantt" && labelScrollRef.current) labelScrollRef.current.scrollTop = scrollTop;
    if (source === "label" && ganttScrollRef.current) ganttScrollRef.current.scrollTop = scrollTop;
  }
    ...
    <div className="flex flex-col h-full bg-white" onClick={() => setPopup(null)}>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="shrink-0 flex flex-col border-r border-gray-200" style={{ width: LABEL_W }}>
          ...
          {/* employee labels — synced scroll */}
          <div className="overflow-y-auto flex-1 overflow-x-hidden" ref={labelScrollRef}
            onScroll={e => syncScroll("label", (e.target as HTMLElement).scrollTop)}>
            {Array.from(departments.entries()).map(([dept, emps]) => (
              <div key={dept}>
                ...
                {emps.map((emp: any) => (
                  <div key={emp.userId}
                    className="flex items-center gap-2 px-3 border-b border-gray-100 bg-white"
                    style={{ height: ROW_H }}>
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="text-xs font-semibold text-[#221F1F] truncate">{emp.userName}</div>
                      {emp.jobTitle && <div className="text-[10px] text-gray-400 truncate">{emp.jobTitle}</div>}
                      <div className="text-[10px] text-gray-300 mt-0.5">
                        {emp.projects.length === 0 ? "Fara proiecte" : `${emp.projects.length} proiect${emp.projects.length > 1 ? "e" : ""}`}
                      </div>
                    </div>
                  </div>
                ))}
        ...
        {/* scrollable gantt */}
        <div className="flex-1 overflow-auto" ref={ganttScrollRef}
          onScroll={e => syncScroll("gantt", (e.target as HTMLElement).scrollTop)}>
              ...
              {todayOffset >= 0 && todayOffset < VIEW_DAYS && (
                <div className="absolute top-0 bottom-0 z-20 pointer-events-none"
                  style={{
                    ...
                    width: 2, background: "#FFCB09", opacity: 0.85,
                  }} />
              )}
              {Array.from(departments.entries()).map(([dept, emps]) => (
                <div key={dept}>
                    ...
                    <div key={emp.userId}
                      ...
                      {dayTicks.filter(d => d.isWeekend).map(d => (
                        <div key={d.offset}
                          ...
                          style={{ left: d.offset * pxPerDay, width: pxPerDay, background: "rgba(0,0,0,0.025)" }} />
                      ))}
                      {/* project bars stacked vertically inside the cell */}
                      {(emp.projects as any[]).slice(0, MAX_BARS_VISIBLE).map((proj: any, projIdx: number) => {
                        ...
                        return (
                          <div key={proj.projectId} title={proj.projectName}
                            className="absolute rounded cursor-pointer flex items-center gap-1 px-1.5 overflow-hidden select-none hover:brightness-95 transition-all"
                            style={{
                              left: barLeft, width: barW, height: BAR_H, top: barTop,
                              background: proj.projectColor || "#FFCB09",
                              border: hasActive ? "1.5px solid #1d4ed8" : "1px solid rgba(0,0,0,0.15)",
                              boxShadow: hasActive ? "0 0 0 2px rgba(29,78,216,0.15)" : "none",
                            }}
                            onClick={e => handleBarClick(e, emp, proj)}>
                            {proj.projectEmoji && barW >= 28 && (
                              <span className="text-[11px] leading-none shrink-0">{proj.projectEmoji}</span>
                            )}
                            {barW >= 36 && (
                              <span className="text-[10px] font-bold truncate leading-none" style={{ color: "#221F1F" }}>
                      ...
                      {/* "+N more" indicator if more than MAX_BARS_VISIBLE projects */}
                      {emp.projects.length > MAX_BARS_VISIBLE && (
                        <div className="absolute right-1 bottom-1 text-[9px] text-gray-400 font-medium bg-white/80 rounded px-1">
                          +{emp.projects.length - MAX_BARS_VISIBLE} mai mult
                        </div>
                      )}
                    </div>
                  ))}
